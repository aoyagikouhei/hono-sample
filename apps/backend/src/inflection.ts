...
let plurals: [RegExp | string, string][] = [];
let singulars: [RegExp | string, string][] = [];
...
const inflector = {
   ...
   */
  gsub: function (word: string, rule: string | RegExp, replacement: string): string | null {
    const source = typeof rule === 'string' ? rule : rule.source;
   ...
   */
  plural: function (rule: string | RegExp, replacement: string): void {
   ...
   */
  pluralize: function (word: string, count?: number, includeNumber?: boolean): string {
    let result: string;

    if (count !== undefined) {
      const parsedCount = parseFloat(count.toString());
      result = (parsedCount === 1) ? this.singularize(word) : this.pluralize(word);
      result = (includeNumber) ? [parsedCount, result].join(' ') : result;
    } else {
      if (_.includes(uncountables, word)) {
        return word;
      }
      ...
      _(plurals).find((rule) => {
        const gsub = this.gsub(word, rule[0], rule[1]);
   ...
   */
  singular: function (rule: string | RegExp, replacement: string): void {
   ...
   * passed to it.
   */
  singularize: function (word: string): string {
    if (_.includes(uncountables, word)) {
      return word;
    }
   ...
   */
  irregular: function (singular: string, plural: string): void {
   ...
   */
  uncountable: function (word: string): void {
   ...
   */
  ordinalize: function (number: number | string): string {
   ...
   */
  titleize: function (words: string): string {
